...
class Wcmultishipping_Blocks_Integration implements IntegrationInterface
{
    ...
    {
        $this->register_pickup_points_block_frontend_scripts();
        $this->register_pickup_points_block_editor_scripts();
    ...
    {
        return array('wcmultishipping-blocks-integration', 'wcmultishipping-checkout-pickup-points-block-frontend');
    }

    public function get_editor_script_handles()
    {
        return array('wcmultishipping-blocks-integration', 'wcmultishipping-checkout-pickup-points-block-editor');
    }
    ...
    public function register_pickup_points_block_editor_scripts()
    {
        $script_path = '/inc/woocommerce_block/wcmultishipping-checkout-pickup-points-block.js';
        $script_url = plugins_url($script_path, __FILE__);
        $script_asset_path = dirname(__FILE__).'/inc/woocommerce_block/wcmultishipping-checkout-pickup-points-block.asset.php';
        $script_asset = file_exists($script_asset_path)
            ? require $script_asset_path
            : array(
                'dependencies' => array(),
        ...
        wp_set_script_translations(
            'wcmultishipping-checkout-pickup-points-block-editor', // script handle
            'wcmultishipping', // text domain
            dirname(__FILE__).'/languages'
        );
    }

    public function register_pickup_points_block_frontend_scripts()
    {
        $script_path = '/inc/woocommerce_block/wcmultishipping-checkout-pickup-points-block-frontend.js';
        $script_url = plugins_url($script_path, __FILE__);
        $script_asset_path = dirname(__FILE__).'/inc/woocommerce_block/wcmultishipping-checkout-pickup-points-block/pickup-points-block-frontend.asset.php';
        $script_asset = file_exists($script_asset_path)
            ? require $script_asset_path
            : array(
                'dependencies' => array(),
        ...
        wp_set_script_translations(
            'wcmultishipping-checkout-pickup-points-block-frontend', // script handle
            'wcmultishipping', // text domain
            dirname(__FILE__).'/languages'
        );
    }
