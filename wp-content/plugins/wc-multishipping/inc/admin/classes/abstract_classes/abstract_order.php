...
abstract class abstract_order
{
    ...
    private function add_order_by($args)
    {
        ...
        if (empty($args['orderby'])) return OrderUtil::custom_orders_table_usage_is_enabled() ? " ORDER BY {$wpdb->prefix}wc_orders.date_created_gmt DESC " : " ORDER BY {$wpdb->prefix}posts.post_date DESC ";
    ...
    public static function get_shipping_method_name($order)
    {
        ...
        if (is_int($order)) $order = new \WC_Order($order);
