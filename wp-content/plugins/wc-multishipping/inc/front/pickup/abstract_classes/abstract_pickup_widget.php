...
abstract class abstract_pickup_widget
{
    ...
    public function select_pickup_point()
    {
        ...
        if (strlen(4 == $pickup_zip_code)) $pickup_country = "0".strval($pickup_zip_code);
    ...
    public function save_pickup_info($order_id)
    {
        ...
        if (!is_int($order_id)) $order_id = $order_id->get_id();
    ...
    public function prevent_place_order_button($order_button)
    {
        ...
        if (!WC()->cart->needs_shipping()) return $order_button;

        $selected_shipping_method = WC()->session->get('chosen_shipping_methods');
        if (empty($selected_shipping_method) || !is_array($selected_shipping_method)) return $order_button;
    ...
    public function prevent_checkout_process($fields = [], $errors = null)
    {
        ...
        $selected_shipping_method = WC()->session->get('chosen_shipping_methods');
        if (empty($selected_shipping_method) || !is_array($selected_shipping_method)) return;
        ...
        if (empty($selected_shipping_method) || !in_array($selected_shipping_method_name, $order_class::ID_SHIPPING_METHODS_RELAY)) return;
        $pickup_info = WC()->session->get(self::PICKUP_LOCATION_SESSION_VAR_NAME);
        if (empty($pickup_info)) {
            if ($errors) $errors->add('validation', esc_html__('Please select a pick-up point', 'wc-multishipping')); else throw new \Exception(esc_html__('Please select a pick-up point', 'wc-multishipping'));
        }
    }
